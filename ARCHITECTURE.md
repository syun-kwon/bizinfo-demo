...
`bizinfo-web` 컨테이너는 MCP 서버의 `server.py`를 `bizinfo_server.py`로 복사한 뒤 Python 모듈로 직접 임포트합니다. 이를 통해 MCP 프로토콜 오버헤드 없이 API 함수를 바로 호출합니다.
...
외부 인터넷
    ...
    │
    └── 포트 8000 (내부 전용) → bizinfo-mcp
              ↑
         Claude Desktop만 접근
         (호스트 머신에서 localhost:8000)
```
...
| 패키지 | 용도 |
|--------|------|
| `mcp` | MCP 프로토콜 서버 프레임워크 (FastMCP) |
| `httpx` | 비동기 HTTP 클라이언트 (API 호출) |
| `pydantic` | 입력 유효성 검사 (v2) |
| `fastapi` | REST API 웹 프레임워크 |
| `uvicorn` | ASGI 서버 |
